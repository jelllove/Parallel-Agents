...
export function terminalKeysForProject(
  ...
  return [agentTerminalKey(projectId), ...shellProfiles.map((profile) => shellTerminalKey(projectId, profile))];
}
...
export function parseSessionTabKey(tabKey: string): { projectId: string; sessionId: string | null } {
