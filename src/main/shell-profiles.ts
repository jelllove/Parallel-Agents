...
const shellNames = new Set([
  'sh', 'bash', 'dash', 'zsh', 'ksh', 'ksh93', 'tcsh', 'csh', 'fish', 'pwsh',
  'nu', 'yash', 'ash', 'elvish', 'xonsh', 'oil', 'osh',
]);
...
export function createShellDiscovery(overrides: Partial<ShellDiscoveryProbes> = {}) {
  const probes: ShellDiscoveryProbes = {
    ...
    execFile(command, args) {
      return new Promise<Buffer>((resolve, reject) => {
        execFile(command, args, {
          encoding: 'buffer', timeout: 3000, maxBuffer: 1024 * 1024, windowsHide: true,
        }, (error, stdout, stderr) => {
  ...
  async function discoverShells(): Promise<AvailableShell[]> {
    const pathDirectories = (env('PATH') ?? '').split(paths.delimiter)
      ...
      .filter((entry) => paths.isAbsolute(entry)).slice(0, 128);
    ...
    if (windows) {
      ...
      await add('powershell', 'Windows PowerShell', [
        paths.join(system32, 'WindowsPowerShell', 'v1.0', 'powershell.exe'), ...onPath('powershell.exe'),
      ]);
      await add('pwsh', 'PowerShell', [
        ...onPath('pwsh.exe'), ...programFiles.map((dir) => paths.join(dir, 'PowerShell', '7', 'pwsh.exe')),
      ]);
      const git = await firstExecutable(onPath('git.exe'));
      const gitRoot = git ? paths.dirname(paths.dirname(git)) : undefined;
      await add('bash', 'Git Bash', [
        // System32/bash.exe is a legacy WSL launcher, not an installed Git Bash.
        ...onPath('bash.exe').filter((command) => paths.dirname(command).toLowerCase() !== system32.toLowerCase()),
        ...programFiles.map((dir) => paths.join(dir, 'Git', 'bin', 'bash.exe')),
        ...(gitRoot ? [
          paths.join(gitRoot, 'bin', 'bash.exe'), paths.join(gitRoot, 'usr', 'bin', 'bash.exe'),
      ...
      if (wsl) {
        ...
        try {
          ...
          const failure = error as NodeJS.ErrnoException & { stdout?: Buffer | string; stderr?: Buffer | string; killed?: boolean };
          const detail = `${decodeOutput(failure.stdout)}\n${decodeOutput(failure.stderr)}\n${failure.message}`;
          const unavailable = /WSL_E_(?:WSL_OPTIONAL_COMPONENT_REQUIRED|WSL_NOT_INSTALLED|DEFAULT_DISTRO_NOT_FOUND)|0x8007019e|no installed distributions|wsl(?:\.exe)? --install/i.test(detail);
        ...
        const distributions = [...new Set(output.split(/\r?\n/).map((line) => line.trim()).filter(Boolean))];
        if (!distributions.length) probes.diagnostic('WSL has no installed distributions.');
        for (const name of distributions) {
          result.push({ id: `wsl:${name}`, label: `WSL: ${name}`, command: wsl, args: ['--distribution', name] });
        }
      }
      return result;
    }

    const listed = (await probes.readShells()).split(/\r?\n/).slice(0, 512)
      .map((line) => line.split('#')[0].trim()).filter((line) => paths.isAbsolute(line));
    const candidates = [
      ...listed,
