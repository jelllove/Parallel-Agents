...
async function git(basePath: string, args: string[], context: string): Promise<string> {
  try {
    ...
    throw new Error(`${context}: ${stderr || (error instanceof Error ? error.message : String(error))}`);
  }
}
...
async function resolveTargetPath(targetPath: string): Promise<string> {
  ...
  while (true) {
    try {
      ...
      if (nextParent === parent) throw new Error(`Cannot access target parent: ${parent}`);
      parent = nextParent;
      continue;
    }
...
function requireOutsideWorkingTree(targetPath: string, workingTree: string): void {
  ...
  if (subpath === '' || (!isAbsolute(subpath) && subpath !== '..' && !subpath.startsWith(`..${sep}`))) {
...
export async function findProjectRepositoryRoot(basePath: string): Promise<string | null> {
  ...
  while (true) {
    try {
      ...
      return normalize(await git(directory, ['rev-parse', '--show-toplevel'], 'Cannot resolve project repository'));
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
    }
...
export async function createProjectWorktree(options: {
  ...
  try {
    ...
    throw new Error(`Cannot access base directory: ${error instanceof Error ? error.message : String(error)}`);
  }

  const workingTree = normalize(await realpath(await git(
    basePath, ['rev-parse', '--show-toplevel'], 'Base directory must be a Git working tree',
  )));
  await git(
    basePath, ['rev-parse', '--verify', '--end-of-options', 'HEAD^{commit}'],
  ...
  const commit = await git(
    basePath, ['rev-parse', '--verify', '--end-of-options', `${startPoint}^{commit}`],
  ...
  await git(
    basePath, ['worktree', 'add', '-b', branch, actualTargetPath, commit],
