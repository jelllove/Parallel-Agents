...
import type { AgentId, RegisteredProject } from '../shared/types';
...
export class PreferencesStore {
  ...
  private async readDisk(): Promise<Preferences> {
    ...
    if (!data || !data.sessionNames || typeof data.sessionNames !== 'object'
      || Array.isArray(data.sessionNames) || !Array.isArray(data.projects)) {
  ...
  async renameSession(agent: AgentId, id: string, title: string): Promise<string> {
    ...
    await this.update((data) => ({
      ...data, sessionNames: { ...data.sessionNames, [`${agent}:${id}`]: name },
    }));
    return name;
  }
  ...
  registerProject(project: RegisteredProject): Promise<void> {
    return this.update((data) => ({
      ...data, projects: [...data.projects.filter((p) => p.id !== project.id), project],
    }));
  }
...
export const preferences = new PreferencesStore(join(homedir(), '.claude', 'parallel-agents-preferences.json'));
