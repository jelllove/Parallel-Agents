...
type Updater = Pick<AppUpdater,
  ...
  'checkForUpdates' | 'quitAndInstall' | 'on' | 'removeListener'>;
...
export class UpdateController {
  ...
  private handlers: [UpdaterEvent, (...args: any[]) => void][] = [];
  ...
  constructor(private readonly options: UpdateOptions) {
    ...
    this.status = {
      ...
      canInstall: false,
      message: options.enabled ? undefined : 'Automatic updates are available only in packaged Windows builds.',
    };
    if (!options.enabled) return;
    this.listen('checking-for-update', () => this.setStatus({ state: 'checking', message: undefined, percent: undefined }));
    ...
    this.listen('download-progress', (progress: { percent: number }) =>
      this.setStatus({ state: 'downloading', percent: Math.min(100, Math.max(0, progress.percent)) }));
    this.listen('update-downloaded', (info: { version: string }) =>
      this.setStatus({ state: 'downloaded', version: info.version, percent: 100, canInstall: true, message: undefined }));
  ...
  start(): void {
    ...
    this.started = true;
    void this.check();
    this.timer = setInterval(() => { void this.check(); }, UPDATE_CHECK_INTERVAL);
    this.timer.unref?.();
  }

  async check(): Promise<UpdateStatus> {
    if (!this.options.enabled || this.checking || this.status.canInstall || this.requestingInstall) return this.getStatus();
    this.checking = true;
  ...
  async install(): Promise<UpdateStatus> {
    if (!this.options.enabled || !this.status.canInstall || this.requestingInstall || this.status.state === 'installing') {
      return this.getStatus();
    }
  ...
  dispose(): void {
    ...
    for (const [event, handler] of this.handlers) this.options.updater.removeListener(event, handler);
    this.handlers = [];
    this.listeners.clear();
  }

  private listen(event: UpdaterEvent, handler: (...args: any[]) => void): void {
    this.options.updater.on(event, handler);
