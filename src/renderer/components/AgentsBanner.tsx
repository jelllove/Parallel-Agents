...
export function AgentsBanner() {
  ...
  return (
    <div className={`agents-banner-region${expanded ? ' expanded' : ''}`}
      ...
      <button className="agents-banner-handle" aria-label="Show CLI tools" aria-expanded={expanded}
          ...
          <button
            ...
            onClick={() => {
              if (!ok && window.api.shell?.openExternal) {
                void window.api.shell.openExternal(a.installUrl).catch((err) => setError(String(err)));
              }
            }}
      ...
      <button className="ab-recheck" onClick={() => {
        setError('');
