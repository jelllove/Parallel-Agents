...
export function AboutDialog({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    window.api.updates.getStatus().then((status) => {
        ...
        <div className="modal-body">
          ...
          <dl className="about-meta">
            ...
            <dd title={__APP_BUILD_INFO__.commit ?? undefined}>
              <code>{__APP_BUILD_INFO__.commit?.slice(0, 12) ?? 'Unavailable (source archive)'}</code>
          ...
          <section aria-label="Application updates">
            <p role="status" aria-live="polite">
              ...
              {update?.state === 'idle' && (update.message ?? 'Updates are checked automatically every 4 hours.')}
              {update?.state === 'checking' && 'Checking GitHub releases…'}
              {update?.state === 'available' && `Downloading version ${update.version}…`}
              {update?.state === 'downloading' && `Downloading version ${update.version}: ${Math.round(update.percent ?? 0)}%`}
            ...
            {update?.state === 'downloading' && (
              <progress aria-label="Update download progress" max={100} value={update.percent ?? 0} />
            )}
            {(updateError || update?.state === 'error') && (
              <p role="alert" style={{ overflowWrap: 'anywhere' }}>{updateError ?? update?.message}</p>
            )}
            <button
              className="btn-secondary"
              disabled={!update || pending || update.state === 'unavailable' || update.canInstall || ['checking', 'available', 'downloading', 'installing'].includes(update.state)}
            ...
            {update && update.state !== 'unavailable' && (
              <p>Updates download in the background. Agents are stopped only when you confirm a restart; quitting normally does not install updates.</p>
            )}
          </section>
