...
export function AgentIcon({ agent, className, size = 16 }: { agent: AgentId; className?: string; size?: number }) {
  const url = agentIconUrl(agent);
  if (agent !== 'codex') return <img src={url} className={className} width={size} height={size} alt="" draggable={false} />;
  return <span className={`${className ?? ''} codex-icon`} aria-hidden="true" style={{
