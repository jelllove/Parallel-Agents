...
export function ProjectDeleteDialog({ projects, onConfirm, onCancel }: {
  projects: Project[];
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all(snapshot.map(async (project) =>
  ...
  return (
    <dialog ref={dialog} className="workflow-dialog project-delete-dialog" aria-labelledby="delete-project-title"
      ...
      <form onSubmit={async (event) => {
        ...
        setError('');
        try { await onConfirm(); }
        catch (err) { setError(err instanceof Error ? err.message : String(err)); setArmed(false); }
        finally { setBusy(false); }
      }}>
        <h2 id="delete-project-title">Delete {snapshot.length === 1 ? 'project history' : `${snapshot.length} project histories`}?</h2>
        ...
        <div className="project-delete-summary">
          {snapshot.map((project) => <section key={project.id}>
            ...
            <details open>
              <summary>{sessions ? `${sessions[project.id].length} session${sessions[project.id].length === 1 ? '' : 's'}` : 'Loading sessions...'}</summary>
              <ul>{sessions?.[project.id].map((session) =>
                <li key={session.id}><span>{session.title || '(Untitled session)'}</span> <code>{session.id}</code></li>,
              )}</ul>
            </details>
          </section>)}
        </div>
        <label className="modal-check">
          <input type="checkbox" autoFocus checked={armed} disabled={!ready || busy}
            onChange={(event) => setArmed(event.target.checked)} />
          <span>I have reviewed these projects and sessions and understand deletion cannot be undone.</span>
        </label>
