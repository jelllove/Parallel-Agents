...
export function UpdateReadyNotice() {
  ...
  return (
    <aside
      ...
      style={{
        position: 'fixed', right: 16, bottom: 40, zIndex: 900,
    ...
    >
      <p role="status" aria-live="polite" style={{ marginTop: 0 }}>
        {installing ? 'Installing update and restarting…' : `Version ${update.version} is ready to install.`}
      </p>
      <p>Your agents will keep running until you confirm a restart.</p>
      {(error || update.state === 'error') && (
        <p role="alert" style={{ overflowWrap: 'anywhere' }}>{error ?? update.message}</p>
      )}
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="btn-primary" disabled={pending || installing} onClick={() => void restart()}>
          Restart to update
        </button>
        <button
      ...
      <p style={{ marginBottom: 0, color: 'var(--text-dim)' }}>You can also install this update later from About.</p>
    </aside>
  );
}
