export function ActionIcon({ name, spinning = false }: { name: 'refresh' | 'cleanup' | 'rename'; spinning?: boolean }) {
  return <svg className={`action-icon${spinning ? ' spinning' : ''}`} viewBox="0 0 24 24" fill="none"
