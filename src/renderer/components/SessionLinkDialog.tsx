...
export function SessionLinkDialog({ request, onLinked }: {
  ...
  useEffect(() => {
    ...
    void useAppStore.getState().loadSessions(projectId).catch((err) => setError(String(err)));
  }, [projectId]);
  ...
  return (
    <dialog ref={dialog} className="workflow-dialog" onCancel={(event) => {
      ...
      <form onSubmit={(event) => {
        ...
        <p>Choose the session running in this terminal. Linking does not start or stop a CLI.
          Confirm its identity before linking: another CLI may have created history in this project.</p>
        <label className="workflow-field">Running session
          <select autoFocus required value={selected} disabled={busy} onChange={(event) => setSelected(event.target.value)}>
            <option value="">Choose a session...</option>
            {list.map((session) => <option key={session.id} value={session.id}>{session.title} ({session.id})</option>)}
          </select>
        </label>
        ...
        <div className="workflow-actions">
          ...
          {request.sessionId && <button type="button" className="btn-secondary" disabled={busy || !selectedSession}
