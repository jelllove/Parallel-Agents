...
| [src/main/projects.ts](src/main/projects.ts)                       | Discover/register supported provider projects, create worktree projects, merge a repository with its linked worktrees per agent, apply saved preferences, remove provider history |
| [src/main/worktree-members.ts](src/main/worktree-members.ts)       | Electron-free map from a merged project id to its member folders, shared by project and session code                                                                              |
...
| `clipboard` | `resolvePaste`                                                                                                                                                                                                                                                                                         | —                        |
| `workspace` | `getOpenTabs`, `setOpenTabs`, `getRecentFolders`, `addRecentFolder`, `isGitRepository`, `getSortOrders`, `setSortOrder`                                                                                                                                                                                | —                        |
...
| `$HOME\.claude\parallel-agents-preferences.json` | Session display names, font preferences, registered manual/worktree projects, recent New Project folders (max 18), the open-tab snapshot restored on launch, and per-panel sort orders |
...
A project folder that is the root of a Git checkout is grouped with every other checkout that
shares its `git rev-parse --git-common-dir`, separately per agent. The merged entry keeps the main
repository's project id and folder, sums session counts, and lists sessions from every member
folder; each session keeps its own `cwd`, and resuming it launches the agent in that folder.
Renaming and deleting a session resolve the member that owns it, and deleting a merged project
removes the provider history of every member. Nested subfolder projects (for example a Copilot
package folder) are never grouped. New worktrees default to `<repo>.worktree\<name>`; the
"latest main" option fetches `origin` and starts from `origin/HEAD` (or `origin/main`/`master`)
without checking out or pulling in the base folder.
