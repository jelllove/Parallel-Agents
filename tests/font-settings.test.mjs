...
async function createStore(config) {
  const result = await build({
    entryPoints: ['src/renderer/store/app-store.ts'], bundle: true, write: false,
    platform: 'node', format: 'cjs', packages: 'external',
    plugins: [{
      name: 'commands-without-images',
      setup(builder) {
        builder.onResolve({ filter: /icons\/agentIcons$/ }, () => ({ path: 'icons', namespace: 'commands' }));
        builder.onLoad({ filter: /.*/, namespace: 'commands' }, () => ({
          contents: 'export const startCommandFor=()=>""; export const resumeCommandFor=()=>""; export const extraPathFor=()=>[];',
        }));
      },
    }],
  });
  ...
  new Function('require', 'module', 'exports', 'window', 'document', result.outputFiles[0].text)(
    require, module, module.exports, { api: { config } },
...
async function componentHarness(name, state, api = {}, dependencies = {}) {
  ...
  const module = { exports: {} };
  new Function('require', 'module', 'exports', 'window', 'ResizeObserver', 'requestAnimationFrame', code)(
    (id) => {
      if (id === 'react') return react;
      if (id === '../store/app-store') return { useAppStore };
      if (id === '../../shared/typography') return { TERMINAL_FONT_FAMILY: 'monospace', DEFAULT_FONT_SIZE: 14, MIN_FONT_SIZE: 10, MAX_FONT_SIZE: 24 };
...
test('terminal changes normal weight live without recreating PTY or changing ANSI bold weight', async () => {
  ...
  const render = await componentHarness('TerminalPane', state, {
    pty: {
      spawn: async () => { spawns++; }, resize: async () => {},
      onData: (listener) => { onData = listener; return () => {}; }, onExit: () => () => {},
    },
  }, {
    '@xterm/xterm': { Terminal: class {
      constructor(options) { this.options = options; this.cols = 80; this.rows = 24; this.written = []; terminals.push(this); }
      loadAddon() {} open() {} attachCustomKeyEventHandler() {} onData() {} focus() {}
...
test('Monaco options follow bold toggles without refetching the diff', async () => {
  ...
  const render = await componentHarness('DiffWindow', state, {
    git: { diff: async () => { requests++; return { oldContent: 'old', newContent: 'new' }; } },
  });
...
test('font picker exposes a controlled bold toggle, disables writes and surfaces errors', async () => {
  ...
  let requested;
  const state = {
    fontSize: 14, fontBold: false, setFontSize: async () => {},
    setFontBold: (value) => { requested = value; return new Promise((_resolve, fail) => { reject = fail; }); },
  };
  ...
  assert.equal(checkbox().props.checked, false);
  assert.equal(find(render(), (node) => node.props?.role === 'alert').props.children, 'Cannot save font setting');
  state.fontBold = true;
