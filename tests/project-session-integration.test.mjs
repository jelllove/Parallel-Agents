...
async function backend(home) {
  const result = await build({
    ...
    plugins: [{
      ...
      setup(builder) {
        builder.onResolve({ filter: /^(electron|os)$/ }, (args) => ({ path: args.path, namespace: 'test-home' }));
...
test('new project survives restart, discovers history under stable ID, and renames persist', async (t) => {
  ...
  await writeFile(join(rollout, 'rollout-test.jsonl'), [
    { type: 'session_meta', payload: { id: 'session-one', cwd: folder, timestamp: '2026-09-10T08:00:00Z' } },
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'Original title' }] } },
  ...
  assert.equal((await restarted.listSessionsForProject(created.id))[0].title, 'Implement workspace picker');
  assert.equal((await restarted.createProject({ agent: 'codex', mode: 'folder', basePath: folder })).id, created.id);
  const copilot = await restarted.createProject({ agent: 'copilot', mode: 'folder', basePath: folder });
  assert.notEqual(copilot.id, created.id);
  await rm(folder, { recursive: true });
  await restarted.deleteMissingProjects([created.id]);
  assert.equal((await restarted.listProjects()).some((p) => p.id === created.id), false);
  assert.equal((await restarted.listProjects()).some((p) => p.id === copilot.id), true);
...
test('Copilot subfolder projects share repository history while preserving launch directories', async (t) => {
  ...
  const gitRoot = (await promisify(execFile)('git', ['-C', root, 'rev-parse', '--show-toplevel'])).stdout.trim();
  const api = await backend(home);
  ...
  await writeFile(join(history, 'events.jsonl'), JSON.stringify({
    type: 'session.start', timestamp: '2026-09-10T08:00:00Z',
