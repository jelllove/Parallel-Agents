...
test('returns all known terminal keys for one tab', () => {
  assert.deepEqual(
    terminalKeysForProject('copilot:C:\\repo', ['powershell', 'bash']),
    [
...
test('parses session tab keys into project and session ids', () => {
  assert.deepEqual(
    parseSessionTabKey('copilot:C:\\repo::session:session-123'),
    { projectId: 'copilot:C:\\repo', sessionId: 'session-123' },
  );
});

test('treats plain project keys as non-session tabs', () => {
  assert.deepEqual(
    parseSessionTabKey('copilot:C:\\repo'),
    { projectId: 'copilot:C:\\repo', sessionId: null },
  );
});
