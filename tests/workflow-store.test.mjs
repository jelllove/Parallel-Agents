...
async function createStore(api) {
    ...
    plugins: [{
      ...
      setup(builder) {
        ...
        builder.onLoad({ filter: /.*/, namespace: 'commands' }, () => ({
          contents: "export const startCommandFor=id=>id; export const resumeCommandFor=(id,s)=>`${id} resume ${s}`; export const extraPathFor=()=>[];",
        }));
      },
    }],
  });
  ...
  new Function('require', 'module', 'exports', 'window', 'document', result.outputFiles[0].text)(
    createRequire(import.meta.url), module, module.exports, { api },
...
test('single session project opens a bound tab and renames without restarting it', async () => {
  ...
  const api = {
    sessions: {
      ...
      rename: async (_project, _id, title) => { session = { ...session, title }; return title; },
    },
  ...
  const project = { id: 'codex:repo', agent: 'codex', dirName: 'repo', realPath: 'C:\\repo', displayName: 'Repo', exists: true, sessionCount: 1 };
...
test('new project requires explicit session association and avoids duplicate resume', async () => {
  let sessions = [];
  const project = { id: 'codex:manual:repo', agent: 'codex', dirName: 'manual:repo', realPath: 'C:\\repo', displayName: 'Repo', exists: true, sessionCount: 0 };
  const store = await createStore({
    projects: { create: async () => project },
    sessions: { listForProject: async () => sessions },
    config: { setLastAgent: async () => {} },
  });
  await store.getState().createProject({ agent: 'codex', mode: 'folder', basePath: project.realPath });
