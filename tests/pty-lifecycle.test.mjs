...
async function createManager(resolveShell) {
  ...
  const result = await build({
    entryPoints: ['src/main/pty-manager.ts'], bundle: true, write: false,
    ...
    plugins: [{
      ...
      setup(builder) {
        const mocks = {
          ...
          './shell-profiles': 'export const resolveAvailableShell=harness.resolveShell;',
          './window-messenger': 'export const sendToWindow=harness.send;',
        };
        builder.onResolve({ filter: /^(node-pty|\.\/shell-profiles|\.\/window-messenger)$/ }, ({ path }) => ({ path, namespace: 'probes' }));
        builder.onLoad({ filter: /.*/, namespace: 'probes' }, ({ path }) => ({ contents: mocks[path] }));
      },
    }],
  });
  const module = { exports: {} };
  new Function('require', 'module', 'exports', 'harness', 'setTimeout', result.outputFiles[0].text)(
    createRequire(import.meta.url), module, module.exports, harness, (callback) => timers.push(callback),
  );
...
test('stale process output, exit and startup commands never affect its replacement', async () => {
  const { manager, processes, events, timers } = await createManager(async () => ({ command: 'bash', args: [] }));
...
test('shell discovery failure propagates and permits a later launch', async () => {
  const { manager, processes } = await createManager(async () => { throw new Error('Shell unavailable'); });
  await assert.rejects(manager.spawn('tab', 'C:\\repo', 80, 24, undefined, undefined, 'missing'), /Shell unavailable/);
