...
const { code } = await transform(await source('src/main/update-controller.ts'), { loader: 'ts', format: 'cjs' });
...
function setup(options = {}) {
  ...
  const controller = new UpdateController({
    updater, enabled: true, currentVersion: '0.1.9',
    confirmAndInstall: async (install) => { install(); return true; },
    ...options,
  });
...
test('download state/progress reaches ready without restarting and blocks redundant checks', async () => {
  ...
  await controller.check();
  assert.equal(updater.checks, 1);
  assert.deepEqual(statuses.map((s) => s.state), ['checking', 'available', 'downloading', 'downloaded']);
...
test('check and automatic download failures are surfaced and retryable', async () => {
  const { updater, controller } = setup();
  updater.checkForUpdates = async () => { throw new Error('GitHub is unreachable'); };
...
test('install requires ready state and confirmation, and cancellation leaves agents untouched', async () => {
  ...
  const { updater, controller } = setup({
    confirmAndInstall: async (install) => {
      confirmations++;
      if (await new Promise((resolve) => { approve = resolve; })) {
        install();
        return true;
      }
      return false;
...
test('install errors remain visible and restore normal application quit handling', async () => {
  let recovered = 0;
  const { updater, controller } = setup({ onInstallError: () => { recovered++; } });
  updater.emit('update-downloaded', { version: '0.2.0' });
...
test('confirmation errors are surfaced without attempting installation', async () => {
  const { updater, controller } = setup({
    confirmAndInstall: async () => { throw new Error('Confirmation window unavailable'); },
  });
  updater.emit('update-downloaded', { version: '0.2.0' });
...
test('preload update API forwards typed operations and removes event subscriptions', async () => {
  const { code } = await transform(await source('src/preload/index.ts'), { loader: 'ts', format: 'cjs' });
  const ipc = new EventEmitter();
  const calls = [];
  ipc.invoke = async (...args) => { calls.push(args); return { state: 'idle' }; };
  let api;
  new Function('require', code)((id) => {
    assert.equal(id, 'electron');
    return { ipcRenderer: ipc, contextBridge: { exposeInMainWorld: (_name, value) => { api = value; } } };
  });
  ...
  let received;
  const unsubscribe = api.updates.onStatus((status) => { received = status; });
...
test('release configuration produces GitHub NSIS update artifacts', async () => {
  ...
  assert.deepEqual(pkg.build.publish, {
    provider: 'github', owner: 'jelllove', repo: 'Parallel-Agents', releaseType: 'release',
  });
...
async function quitHarness({ tabs = ['Claude agent', 'PowerShell'], response = 1, rendererFailed = false } = {}) {
  const calls = [];
  ...
  new Function('require', 'module', 'exports', code)((id) => {
    if (id === 'electron') return {
      app,
      dialog: { showMessageBox: async (_win, options) => { confirmation = options; return { response }; } },
    };
    if (id === './pty-manager') return { ptyManager: {
      detachWindow: () => calls.push('detach-pty'), killAll: () => calls.push('kill-agents'),
    } };
    if (id === './git') return {
      detachWindow: () => calls.push('detach-git'), unwatchAll: () => calls.push('unwatch'),
    };
...
test('real quit confirmation cancels updates without killing PTYs, and approves cleanup before installation', async () => {
  for (const response of [1, 0]) {
    ...
    assert.deepEqual(h.calls, response === 0 ? ['detach-pty', 'detach-git', 'unwatch', 'kill-agents', 'install'] : []);
  }
});
...
test('About dialog shows download progress, install errors and explicit restart while preserving build metadata', async () => {
  ...
  const react = {
    ...
    useEffect(effect) {
      const index = cursor++;
      if (!slots[index]) { slots[index] = {}; effects.push(effect); }
    },
  };
  const api = { updates: {
    getStatus: async () => ({ state: 'idle', currentVersion: '0.1.9', canInstall: false }),
    onStatus: (listener) => { onStatus = listener; return () => { unsubscribed = true; }; },
    install: async () => { installs++; throw new Error('Restart failed'); },
  } };
  const { code } = await transform(await source('src/renderer/components/AboutDialog.tsx'), {
    loader: 'tsx', format: 'cjs', jsx: 'automatic',
  });
  const module = { exports: {} };
  new Function('require', 'module', 'exports', 'window', '__APP_BUILD_INFO__', code)((id) => {
    if (id === 'react') return react;
    if (id === './ClaudeIcon') return { ClaudeIcon: 'icon' };
    return require(id);
  }, module, module.exports, { api, addEventListener() {}, removeEventListener() {} },
  { version: '0.1.9', commit: '1234567890abcdef', dirty: false });
  const render = () => { cursor = 0; return module.exports.AboutDialog({ onClose() {} }); };
  render();
  ...
  assert.equal(find(render(), (node) => node.props?.children === 'Check for updates').props.disabled, true);
  ...
  assert.equal(find(render(), (node) => node.props?.role === 'alert').props.children, 'Restart failed');
...
test('ready notice is visible without About, never auto-installs, supports Later and reports explicit install errors', async () => {
  ...
  const api = { updates: {
    ...
    onStatus: (listener) => { onStatus = listener; return () => { unsubscribed = true; }; },
    install: async () => { installs++; return ready; },
  } };
  ...
  new Function('require', 'module', 'exports', 'window', code)(
    (id) => id === 'react' ? react : require(id), module, module.exports, { api },
  );
  const render = () => { cursor = 0; return module.exports.UpdateReadyNotice(); };
  ...
  api.updates.install = async () => { throw new Error('Confirmation unavailable'); };
  find(render(), (node) => node.props?.children === 'Restart to update').props.onClick();
  await Promise.resolve();
  await Promise.resolve();
  assert.equal(find(render(), (node) => node.props?.role === 'alert').props.children, 'Confirmation unavailable');
  cleanups.forEach((cleanup) => cleanup?.());
  assert.equal(unsubscribed, true);
});
